...
def build_analysis_prompt(market_data: dict, signals_data: list[dict], portfolio: dict, config: dict) -> str:
    """분석용 유저 프롬프트 생성"""
    ...
    lines.append(f"- 총 손익: {summary['total_pnl']:+,.0f}원 ({summary['total_pnl_pct']:+.2f}%)")
    lines.append(f"- 보유 현금: {summary['cash']:,.0f}원 (현금 비중 {summary['cash_pct']:.1f}%)")
    ...
    if positions:
        ...
        for pos in positions:
            ...
            lines.append(f"- 투자금: {pos['entry_price'] * pos['shares']:,.0f}원")
            ...
            if current:
                ...
                lines.append(f"- 평가손익: {pnl_amt:+,.0f}원 ({pnl_pct:+.2f}%)")
                lines.append(f"- 평가금액: {pos.get('market_value', 0):,.0f}원")
    ...
    non_portfolio_signals = [s for s in signals_data if s["ticker"] not in portfolio_tickers]
    ...
    if non_portfolio_signals:
        ...
        for item in non_portfolio_signals:
            ...
            lines.append(f"- 5일: {sig['change_5d']:+.2f}% / 20일: {sig['change_20d']:+.2f}%")

            s = sig["signals"]
            lines.append(f"- RSI: {s['rsi']['value']:.1f} ({'과매수' if s['rsi']['overbought'] else '과매도' if s['rsi']['oversold'] else '중립'})")
            lines.append(f"- MACD: {s['macd']['histogram']:.2f}")
            lines.append(f"- BB 압축: {s['bb_compression']['percentile']:.0f}%ile")

            if "fundamentals" in item:
                f = item["fundamentals"]
                lines.append(f"- PER: {f.get('per', 'N/A')} / PBR: {f.get('pbr', 'N/A')} / 배당: {f.get('div_yield', 'N/A')}%")
    ...
    if summary["cash"] > 0 and non_portfolio_signals:
        ...
        for item in non_portfolio_signals:
            ...
            max_shares = int(summary["cash"] // price)
            if max_shares > 0:
                lines.append(f"- {item['name']}: 현재가 {price:,.0f}원 기준 최대 **{max_shares}주** 매수 가능 ({price * max_shares:,.0f}원)")

    lines.append("\n---")
    lines.append("위 데이터를 기반으로 내 포트폴리오 중심의 오늘 투자 전략을 분석 구조에 따라 출력해주세요.")
    lines.append("**필수**: 모든 매수/매도 추천에 반드시 **수량(주)**과 **금액(원)**을 산술적으로 계산하여 제시하세요.")
    lines.append("분할 매수/매도 시 1차/2차 수량, 목표가, 소요/회수 금액을 구체적으로 나누어 제시하세요.")
