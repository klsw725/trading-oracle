...
def _screen_us_stocks(market: str = "NASDAQ", top_n: int = 30) -> list[dict]:
    """미국 주도주 스크리닝. 시총 상위 모멘텀 필터링."""
    exchange = "NASDAQ" if market in ("NASDAQ", "US") else "NYSE"
    listing = fdr.StockListing(exchange)
    if listing.empty:
        return []

    # 시총 상위 (IndustryCode 기반 필터 없이 단순 정렬)
    ...
    for _, row in listing.iterrows():
        ...
        try:
            ...
            fund = fetch_naver_fundamentals(ticker)  # US는 빈 dict 반환
            per = fund.get("per", 0)
            pbr = fund.get("pbr", 0)
            div_yield = fund.get("div_yield", 0)
