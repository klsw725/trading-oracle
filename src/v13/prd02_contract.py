...
from src.v13.prd02_probes import run_probes
...
def run_acceptance() -> Prd02AcceptanceReport:
    ...
    state: Literal["pass", "fail"] = "pass" if all(checks.values()) and all(
        value == "killed" for value in mutations.values()) else "fail"
    ...
    body: JsonValue = {"state": state, "scenario_count": len(checks),
        "mutation_count": len(mutations), "checks": checks_json,
        "mutations": mutations_json, "hashes": hashes_json}
    return Prd02AcceptanceReport(state=state, scenario_count=len(checks),
