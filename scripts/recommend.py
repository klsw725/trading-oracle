...
def main():
    ...
    parser.add_argument("--market", default="ALL", help="시장 (ALL, KOSPI, KOSDAQ, US, NASDAQ, NYSE)")
    parser.add_argument("--top", type=int, default=6, help="스크리닝 후보 수 (기본: 6)")
    parser.add_argument("--no-filter", action="store_true", help="시그널 필터 없이 전체 분석")
    ...
    console.print(f"  스크리닝: {result['screened']}개 후보 → 시그널 필터: {result['signal_filtered']}개 통과 → 분석: {result['analyzed']}개")
    console.print()
    ...
    if not recs:
        ...
        if regime.get("regime") == "bear":
            console.print("  [dim]시장 레짐이 하락 추세입니다. 현금 비중 유지를 권장합니다.[/dim]")
        console.print()
        return
    ...
    for rec in recs:
        ...
        lines = []
        lines.append(f"[bold]시그널:[/bold] {sig['verdict']} (Bull {sig['bull_votes']}/6)")

        if consensus:
            lines.append(f"[bold]합의도:[/bold] {consensus['consensus_label']} ({consensus['consensus_verdict']})")
            lines.append("")
